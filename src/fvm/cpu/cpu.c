	...
	switch(Memory[CPU_regs->IP])
		{
			...
			case FVM_WRITE_PPU:
				printf("src: %x, off : %d, len : %d\n", (int)((uintptr_t)Memory + Memory[CPU_regs->r12+3]), (int)Memory[CPU_regs->r12+1], (int)Memory[CPU_regs->r12+2]);
				CPU_regs->IP++;

				write_ppu(FVM_PPU, (uint8_t*)Memory, Memory[CPU_regs->r12+3], Memory[CPU_regs->r12+1], Memory[CPU_regs->r12+2]);
				break;
			case FVM_READ_PPU:
				CPU_regs->IP++;
